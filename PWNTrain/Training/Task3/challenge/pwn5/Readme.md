...
Bài này chương trình cũng có các hàm sử dụng để tương tác với heap

![](./img/editheap.png)

![](./img/one_byte_null.png)

Để ý rằng hàm `readSTr` sẽ thêm byte `0` ở cuối string mà ta nhập vào, tuy nhiên nếu ta nhập hết số byte mà ta có thể thì byte `0` sẽ bị chèn xuống `chunk size` bên dưới => bug `poison null byte`.
