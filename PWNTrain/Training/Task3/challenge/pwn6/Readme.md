...
Bài này sử dụng `glibc 2.28` nên đã có `tcache bin` đồng thời xuất hiện bug `use after free` => Ý tưởng bài này của mình là overwrite `__free_hook` thành `system()` sau đó free một chunk có chứa string `/bin/sh\0`
